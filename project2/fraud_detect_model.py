...
x_train, x_temp, y_train, y_temp = train_test_split(x_train, y_train, train_size=0.1, random_state=77)
x_test, x_temp, y_test, y_temp = train_test_split(x_test, y_test, train_size=0.1, random_state=77)
...
model = RandomizedSearchCV(xgb, params, n_jobs=-1, cv=kfold, scoring=scoring, n_iter=10, verbose=1, refit='AUC', return_train_score=True, random_state=77)
...
# model.get_booster().feature_names = index
...
model = RandomizedSearchCV(xgb, params, n_jobs=-1, cv=kfold, verbose=1, scoring=scoring, n_iter=3, refit='AUC', return_train_score=True, random_state=77)
