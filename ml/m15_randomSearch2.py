...
model = RandomizedSearchCV(RandomForestClassifier(), params, cv=kfold)


# 3. 훈련
model.fit(x_train,y_train) #model: GridRandomizedSearchCVSearch
...
min_samples_split=5,
                       n_jobs=-1)
최적 하이퍼 파라미터 :  {'n_jobs': -1, 'n_estimators': 100, 'min_samples_split': 5, 'min_samples_leaf': 3, 'max_depth': 6}
최고 정확도 : 0.9582
(테스트 데이터 세트 정확도) 최종정답률 :  0.9649122807017544
'''